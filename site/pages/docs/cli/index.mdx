...
<Tabs>
  <Tab label="Windows">
    ...
    4. 回顾 [创建API令牌](/docs/register/4-token)，在 Cresc AI 中创建 **CC** 分组的令牌，替换上方 `xxx`部分
  ...
  <Tab label="MacOS">
    ...
    4. 回顾 [创建API令牌](/docs/register/4-token)，在 Cresc AI 中创建 **CC** 分组的令牌，替换上方 `xxx`
...
<Tabs>
  <Tab label="Windows">
    ...
    回顾 [创建API令牌](/docs/register/4-token) 这一步教程，我们需要在Cresc AI中创建 **Codex** 分组的令牌，然后点击右侧的复制按钮，将key填入“xxx”部分后保存
